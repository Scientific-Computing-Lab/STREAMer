...
//pmem stuff
//#define BILLION 1E9
#define PMEM_MAX_SIZE (1073741824)  // 1 Gigabyte
//#define PMEM_MAX_SIZE (1024 * 1024 * 32)

//static char path[100]="/mnt/pmem1/sveingf-file";
...
//Commenting out old arrays that can't be allocated to nvdimm
/*
...
//Normal pointers that can be used in both dimm and nvdimm
...
//	const char path[] = "/mnt/pmem0-xfs/pool.obj";
        char path[512];
        if (argc !=2){
           printf("Needto provide the dax path (to create the pool object)");
           return 1;
        }
        else{
           strcpy(path,argv[1]);
    	...
    	for (k=0; k<NTIMES; k++)
	{
		...
		#pragma omp parallel for
			for (j=0; j<STREAM_ARRAY_SIZE; j++)
	    			D_RW(a)[j] = D_RO(b)[j]+scalar*D_RO(c)[j];
		#endif
...
void checkSTREAMresults ()
{
	...
	cSumErr = 0.0;
	for (j=0; j<STREAM_ARRAY_SIZE; j++) {
		aSumErr += abs(D_RO(a)[j] - aj);
		bSumErr += abs(D_RO(b)[j] - bj);
		cSumErr += abs(D_RO(c)[j] - cj);
	...
	if (abs(aAvgErr/aj) > epsilon) {
		...
		ierr = 0;
		for (j=0; j<STREAM_ARRAY_SIZE; j++) {
			if (abs(D_RW(a)[j]/aj-1.0) > epsilon) {
				ierr++;
				#ifdef VERBOSE
					if (ierr < 10) {
						printf("         array a: index: %ld, expected: %e, observed: %e, relative error: %e\n",
							j,aj,D_RO(a)[j],abs((aj-D_RO(a)[j])/aAvgErr));
					}
				#endif
			}
		}
	...
	if (abs(bAvgErr/bj) > epsilon) {
		...
		ierr = 0;
		for (j=0; j<STREAM_ARRAY_SIZE; j++) {
			if (abs(D_RO(b)[j]/bj-1.0) > epsilon) {
				ierr++;
				#ifdef VERBOSE
					if (ierr < 10) {
						printf("         array b: index: %ld, expected: %e, observed: %e, relative error: %e\n",
							j,bj,D_RO(b)[j],abs((bj-D_RO(b)[j])/bAvgErr));
				}
				#endif
			}
		}
	...
	if (abs(cAvgErr/cj) > epsilon) {
		...
		ierr = 0;
		for (j=0; j<STREAM_ARRAY_SIZE; j++) {
			if (abs(D_RO(c)[j]/cj-1.0) > epsilon) {
				ierr++;
				#ifdef VERBOSE
					if (ierr < 10) {
						printf("         array c: index: %ld, expected: %e, observed: %e, relative error: %e\n",
							j,cj,D_RO(c)[j],abs((cj-D_RO(c)[j])/cAvgErr));
					}
				#endif
			}
		}
